...
import {
  switchAddFamilyModale,
  switchVegeInfoModale,
} from "../store/slices/vegetableSlice";
import { Audio } from "react-loader-spinner";
import {
  toggleAddFamilyModale,
  toggleAddZoneModale,
  toggleDeleteZoneModale,
...
function Potager(props) {
  ...
  const vegetableInfosModaleSwitch = useSelector(
    (state) => state.vegetable.vegeInfoSwitch
  );
  const SelectedVegetable = useSelector(
    (state) => state.vegetable.vegetableSelected
  );
  ...
  const deleteZoneModale = useSelector(
    (state) => state.modale.deleteZoneModale
  );
  ...
  const [invalidVegetableFormModale, setInvalidVegetableFormModale] =
    useState(false);
  ...
  const SubmitVegetable = async (e) => {
    ...
    const vegetable = vegetableFamily.find(
      (element) => element.name === oneFamily
    );
    ...
    if (
      Date.parse(VegetableObj.start_date_period_seeding) >
        Date.parse(VegetableObj.end_date_period_seeding) ||
      Date.parse(VegetableObj.start_date_period_seeding) >
        Date.parse(VegetableObj.start_date_period_planting) ||
      Date.parse(VegetableObj.start_date_period_planting) >
        Date.parse(VegetableObj.start_date_period_harvest) ||
      Date.parse(VegetableObj.start_date_period_planting) >
        Date.parse(VegetableObj.end_date_period_planting) ||
      Date.parse(VegetableObj.start_date_period_harvest) >
        Date.parse(VegetableObj.end_date_period_harvest)
    ) {
      // si deux dates ne sont pas cohérentes entre elles on affiche un message d'erreur
      setInvalidVegetableFormModale(true);
      setVegetableFormError(
        "les dates ne sont pas valides (les dates de debut et de fin doivent être cohérentes)"
      );
      return;
    }
    ...
    if (
      VegetableObj.zoneId &&
      VegetableObj.familyId &&
      VegetableObj.growthTime &&
      VegetableObj.start_date_period_seeding &&
      VegetableObj.end_date_period_seeding &&
      VegetableObj.start_date_period_planting &&
      VegetableObj.end_date_period_planting &&
      VegetableObj.start_date_period_harvest &&
      VegetableObj.end_date_period_harvest
    ) {
      ...
      let newArray = [];
      if (array) {
        newArray = [...array, VegetableObj];
      } else {
        newArray = [VegetableObj];
      }
  ...
  return (
    <>
      {localStorage.name ? (
        <>
          <main
            className="potager-container"
            onClick={(e) => {
              console.log(e.target.className);
              if (
                !(e.target.className == "VegeInfoSwitch") &&
                vegetableInfosModaleSwitch
              ) {
                dispatch(switchVegeInfoModale(false));
              }
            }}
          >
            <h1 className="title">Mon Potager</h1>
            ...
            <section className="zone-container">
              ...
              {vegetableInfosModaleSwitch ? (
                <>
                  <div className="VegeInfoSwitch">
                    <h3>
                      <strong>{selectedFamily.name}</strong>
                    </h3>

                    <ul style={{ listStyle: "none" }}>
                      {SelectedVegetable.variety ? (
                        <li>variété : {SelectedVegetable.variety}</li>
                      ) : null}
                      <li>
                        temps de croissance: {SelectedVegetable.growth_time}{" "}
                        jours
                      </li>
                    </ul>
                    <div className="tasks">
                      <h4>tâches à faire</h4>
                      {SelectedVegetable.task[0].type == "seeding" ? (
                        <p>semer: {SelectedVegetable.task[0].status}</p>
                      ) : (
                        <p>planter: {SelectedVegetable.task[0].status} </p>
                      )}
                      {SelectedVegetable.task[1].type == "planting" ? (
                        <p>planter: {SelectedVegetable.task[1].status} </p>
                      ) : (
                        <p>recolter: {SelectedVegetable.task[1].status} </p>
                      )}

                      {SelectedVegetable.task[2] ? (
                        <>recolter: {SelectedVegetable.task[2].status} </>
                      ) : null}
                    </div>
                    <div className="familyInfos">
                      <h4>Informations sur la famille de légume</h4>
                      <p>profondeur: {selectedFamily.depth}</p>
                      <p>exposition: {selectedFamily.exposure}</p>
                      <p>
                        espacement entre les rangés:{" "}
                        {selectedFamily.row_spacing}
                      </p>
                      <p>
                        espacement entre les plants: {selectedFamily.spacing}
                      </p>
                      <p>type de sol: {selectedFamily.soil_type}</p>
                    </div>
                    <button
                      onClick={(e) => {
                        HandleDeleteVegetable(e);
                      }}
                    >
                      supprimer le plant
                    </button>
            ...
            {vegetableSwitch ? (
              <>
                <div className="vegetableModale">
                  <h2>Choissisez une famille de légumes</h2>
                      ...
                      return (
                        <>
                          <div className="FamilyToChoose">
                            <img
                              src={`image-graphiste/imglegumes/legume-${e.name.toLowerCase()}-200.webp`}
                              alt="logo laMainVerte"
                              className="vegetableImg"
                            />
                            <button
                              className="family"
                              onClick={(e) => {
                                e.preventDefault();
                                dispatch(switchAddFamilyModale(false));
                                addVegetable(e);
                              }}
                            >
                              {e.name}
        ...
        sur le bouton d'un légume dans la modale de selection de légume */}
                  ...
                  >
                    ...
                    <label htmlFor="">
                      ...
                      <input
                        type="text"
                        placeholder=""
                        onChange={(e) => setOneVariety(e.target.value)}
                      />
                    </label>
                    ...
                    {!(startDateSeeding == 1) ? (
                      <>
                        {" "}
                        <label htmlFor="seeding">
                          * période de semis (début/fin):{" "}
                          <div className="creneau">
                            <input
                              type="date"
                              value={startDateSeeding}
                              onChange={(e) => {
                                setStartDateSeeding(e.target.value);
                              }}
                            />
                            <input
                              type="date"
                              value={endDateSeeding}
                              onChange={(e) => {
                                setEndDateSeeding(e.target.value);
                              }}
                            />
                          </div>
                        </label>
                    ...
                    <label htmlFor="seeding">
                      * période de plantation (début/fin):
                      <div className="creneau">
                        <input
                          type="date"
                          value={startDatePlanting}
                          onChange={(e) => {
                            setStartDatePlanting(e.target.value);
                          }}
                        />
                        <input
                          type="date"
                          value={endDatePlanting}
                          onChange={(e) => {
                            setEndDatePlanting(e.target.value);
                          }}
                        />
                      </div>
                    </label>
                    <label htmlFor="seeding">
                      * période de récolte (début/fin):
                      <div className="creneau">
                        <input
                          type="date"
                          value={startDateHarvest}
                          onChange={(e) => setStartDateHarvest(e.target.value)}
                        />
                        <input
                          type="date"
                          value={endDateHarvest}
                          onChange={(e) => setEndDateHarvest(e.target.value)}
                        />
                      </div>
                    </label>
                    <label htmlFor="seeding">
                      * Temps de croissance (durée total de croissance):
                      <div className="creneau">
                        <input
                          className="growthTimeInput"
                          type="number"
                          value={growthTime}
                          onChange={(e) => {
                            setGrowthTime(parseInt(e.target.value));
                          }}
                        />{" "}
                        <p className="growthTimeInputLabel">jours</p>
                      </div>
                    </label>
                    {!(startDateSeeding == 1) ? (
                      <label htmlFor="seeding">
                        * emergence (temps de croissance du semis):
                        <div className="creneau">
                          <input
                            className="growthTimeInput"
                            type="number"
                            value={emergenceTime}
                            onChange={(e) => {
                              setEmergenceTime(e.target.value);
                            }}
                          />{" "}
                          <p className="growthTimeInputLabel">jours</p>
                        </div>
                    ...
                    <p style={{ fontSize: "0.8rem" }}>
                      * tout les champ annoté par un * sont obligatoire
                    </p>
                    {invalidVegetableFormModale ? (
                      <>
            ...
            <section className=".zone-add-button">
              {addZoneModale ? (
                <div className="zoneModale">
                  <form className="ModalForm" action="">
                    {sameNameModale ? (
                      <>
                        <span className="sameNameMessage">
                          Vous avez déja une zone à ce nom
                        </span>
                      </>
                    ) : null}
                    {emptyNameModale ? (
              ...
              {vegetableSwitch || addVegetableModale ? null : (
                <>
                  <button
                    className="zoneToAdd"
                    onClick={() => {
                      dispatch(toggleAddZoneModale(true));
                    }}
                  >
                    +
                  </button>
        ...
        <>
          <div className="NoTokenpage">
            <h2>veuillez vous créer un compte ou vous connecter :)</h2>
          </div>
        </>
      )}
    </>
